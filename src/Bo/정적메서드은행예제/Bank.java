...
public class Bank {
    ...
    private int account; // 잔액 표시, 인스턴트 필드(객체가 생성될때 함께 생성 됨)
    ...
    public void setDeposit(final int dep) { // final을 붙힌 이유는 들어온 값이 변경되면안되기에 (입금 금액)
        account += dep; // 잔액에 입금 금액을 더해서 잔액을 누적
        System.out.println(dep + "을 입금했습니다.");
    }

    public void setWithdarw(final int with) {
        if(with > account) {
