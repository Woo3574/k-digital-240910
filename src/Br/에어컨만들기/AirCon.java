...
public class AirCon {
    ...
    public AirCon() {
        final int[] monthTemp = {-5, 3, 10, 15, 22, 28, 32, 30, 24, 16, 8, 1};
        Calendar now = Calendar.getInstance();
        int month = now.get(Calendar.MONTH); // 배열의 인덱스 동일
        curTemp = monthTemp[month]; // 해댕 월에 대한 온도를 가져옴
        isPower = false;
        setTemp = 18;
        isCooler = false;
        isHeater = false;
    ...
    public void setAirConState() {
        ...
        if (curTemp > setTemp) { //현재가 더운 상태, 온도를 내리라는 의미
            ...
            isCooler = true;
            isHeater = false;
        } else if (curTemp < setTemp) { // 현재가 추운 상태
            isCooler = false;
            isHeater = true;
        } else {
            isCooler = false;
    ...
    public void setCurTemp(int n) { // 현재온도는 현재 온도에서 입력된 값의 크기만큼 변경 해서 다시 대입
        this.curTemp += n;
    }
